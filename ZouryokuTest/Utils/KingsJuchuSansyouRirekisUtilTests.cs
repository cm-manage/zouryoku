...
{
    [TestClass]
    public class KingsJuchuSansyouRirekisUtilTests : BaseInMemoryDbContextTest
    {
        // ---------------------------------------------------------------------
        // MaintainKingsJuchuSansyouRirekiAsync Tests
        // ---------------------------------------------------------------------
        // -----------------------------------------------------
        // 正常系テストケース
        // -----------------------------------------------------
        ...
        ///     受注参照履歴が50件を超えない場合、参照履歴を追加しても全ての履歴を保持する。
        ///     受注参照履歴が50件を超える場合、参照履歴を追加すると古い履歴が削除される。
        /// </summary>
        [TestMethod]
        [DataRow(KingsJuchuSansyouRirekisUtil.MaxHistoryCount - 1, false, DisplayName = "受注参照履歴が50件以下 → 削除なし")]
        [DataRow(KingsJuchuSansyouRirekisUtil.MaxHistoryCount, true, DisplayName = "受注参照履歴が50件を超過 → 古い履歴削除")]
        public async Task MaintainKingsJuchuSansyouRirekiAsync_更新対象のレコードが存在しない_参照履歴を追加(
            int existingCount,
            bool shouldDeleteOldest)
        {
            ...
            var juchu = new KingsJuchu
            {
                ...
                EntYmd = DateOnly.Parse("2025/01/02"),
                Bukken = "物件",
                JucKin = 100000,
                ChaYmd = DateOnly.Parse("2025/01/03"),
                ProjectNo = "12345-123456",
                SekouBumonCd = "01234",
                HiyouShubetuCd = 1,
                HiyouShubetuCdName = "種別A",
                IsGenkaToketu = false,
                Nendo = 2025,
                BusyoId = 1,
                SearchBukken = "物件",
            ...
            {
                return new KingsJuchuSansyouRireki
                {
                    Id = i,
                    KingsJuchuId = 0, // ダミー
            ...
            // ---------- Assert ----------
            // 登録された履歴を取得
            var registeredRireki = await db.KingsJuchuSansyouRirekis
                .FirstOrDefaultAsync(x => x.KingsJuchuId == juchu.Id && x.SyainBaseId == syainBase.Id);
            ...
            await AssertRirekiCountAsync(syainBase.Id, KingsJuchuSansyouRirekisUtil.MaxHistoryCount);
        ...
        ///     受注参照履歴が50件を超えない場合、参照履歴を更新しても全ての履歴を保持する。
        ///     受注参照履歴が50件を超える場合、参照履歴を更新すると古い履歴が削除される。
        /// </summary>
        [TestMethod]
        [DataRow(KingsJuchuSansyouRirekisUtil.MaxHistoryCount - 1, false, DisplayName = "受注参照履歴が50件以下 → 削除なし")]
        [DataRow(KingsJuchuSansyouRirekisUtil.MaxHistoryCount, true, DisplayName = "受注参照履歴が50件を超過 → 古い履歴削除")]
        public async Task MaintainKingsJuchuSansyouRirekiAsync_登録済みの受注情報の参照履歴がある_参照履歴を更新(
            int existingCount,
            bool shouldDeleteOldest)
        {
            // ---------- Arrange ----------
            ...
            // シード: 更新対象の受注参照履歴
            var existingRireki = new KingsJuchuSansyouRireki
            {
                Id = 100,
                KingsJuchuId = juchu.Id,
                SyainBaseId= syainBase.Id,
        ...
        /// </summary>
        /// <param name="kingsJuchuSansyouRireki">確認対象の受注参照履歴</param>
        /// <param name="beforeUpdateTime">更新前の時間</param>
        /// <param name="afterUpdateTime">更新後の時間</param>
        private static void AssertSansyouTime(
            KingsJuchuSansyouRireki kingsJuchuSansyouRireki,
            DateTime beforeUpdateTime,
            DateTime afterUpdateTime)
        {
            Assert.IsTrue(beforeUpdateTime <= kingsJuchuSansyouRireki.SansyouTime && kingsJuchuSansyouRireki.SansyouTime <= afterUpdateTime,
                "参照時間が正しく更新されていません。");
        }
        ...
        /// </summary>
        /// <param name="kingsJuchuSansyouRirekis">確認対象の受注参照履歴リスト</param>
        private async Task AssertOldestRirekiDeletedAsync(List<KingsJuchuSansyouRireki> kingsJuchuSansyouRirekis)
        {
            var oldestRireki = kingsJuchuSansyouRirekis.OrderBy(x => x.SansyouTime).First();
