...
namespace ZouryokuTest.Pages.KokyakuJohoHyoji
{
    ...
    public class IndexModelKokyakuViewModelTests : IndexModelTestBase
    {
        ...
        /// テスト用のエンティティを作成する
        /// </summary>
        /// <returns>リレーションを含む顧客会社のエンティティ</returns>
        private KokyakuKaisha CreateEntityForViewModelTest()
        {
            // 顧客会社
            var kokyaku = new KokyakuKaishaBuilder()
                .WithId(KokyakuId)
                .WithCode(KokyakuCode)
                .WithName(KokyakuName)
                .WithNameKana(KokyakuNameKana)
                .WithRyakusyou(Ryakusyou)
                .Build();

            // 業種
            kokyaku.Gyousyu = new GyousyuBuilder()
                .WithId(GyousyuId)
                .WithName(GyousyuName)
                .Build();

            // 社員BASE
            kokyaku.EigyoBaseSyain = new SyainBasisBuilder()
                .WithId(EigyouBaseSyainId)
                .Build();

            // 社員マスタ
            kokyaku.EigyoBaseSyain.Syains.Add(new SyainBuilder()
                .WithSyainBaseId(EigyouBaseSyainId)
                .WithName(EigyouSyainName)
                .Build());
        ...
        {
            // ================ Arrange ================ //
            var kokyaku = CreateEntityForViewModelTest();
